...
parser.add_argument("--gan_type", type=str, default='naive',
                    choices=['naive', 'wasserstein', 'wasserstein-gp', 'wasserstein-div'])
...
if __name__ == '__main__':
    ...
    model.set_logger()
    model.load_pretrain_cpt('model/{}_img_pretrain_checkpt_224.pkl'.format(args.dataset),
                            'img', only_weight=True)
    model.load_pretrain_cpt('model/{}_txt_pretrain_checkpt_124.pkl'.format(args.dataset),
                            'txt', only_weight=True)  # 139
    # model.load_cpt('/home/data/nuswide_checkpt_19_3300.pkl')

    def test_by_modal(modal='img'):
        train_embedding, train_target = model.embedding_pred(
            model.train_loader_ordered, modal, specific=True)
        test_embedding, test_target = model.embedding_pred(
            model.test_loader, modal, specific=True)
        kmeans = KMeans(config['n_clusters'], max_iter=600,
